...
def chart_date_read(chart_df, start_date, end_date):
...
class total_chart(Resource):
    def post(self):
        ...
        df = pd.read_csv('../크롤링/샘플차트.csv')
        df = chart_date_read(df, start_date, end_date)
        ...
        if r['region']:
            ...
            artist = r['artist']
            artist_df = df.loc[df['artist'] == artist]
            ...
            artist_result = chart_artist(df)
            title_result = chart_track(df)
            region_result = chart_region(df)
            daterange_result = chart_daterange(df)
...
@api.route('/region_chart')
class chart_read(Resource):
    def get(self):
        df = chart_date_read(read_chart(), '2020-08-01', '2020-09-01')
...
@api.route('/date_chart')
class chart_read(Resource):
    def get(self):
        df = chart_date_read(read_chart(), '2020-08-01', '2020-09-01')

        return make_response(jsonify(chart_date(df)),200)
